-- ============================================================================
-- File   : SQL script/refactor_sp_KPIListDataCollection_html_20260521.sql
-- Scope  : Refactor dbo.sp_KPIListDataCollection_html from DB source
-- Date   : 2026-05-21
-- Note   : User review/run manually. Agent does not execute DB writes.
--          Liquid Glass style is intentionally not used. Renderer uses current
--          dbo.sp_MainStyleCSSParadise and safe tblHtmlScriptCache MERGE.
-- ============================================================================
...
CREATE OR ALTER PROCEDURE [dbo].[sp_KPIListDataCollection_html]
(
    @LoginID INT = 3,
...
try {
  ...
  InstanceCompanySize_IDPF09EBAE4C6DA4BE3B1A79BAF021AC73C._suppressValueChangeAction();
    ...
    USING (SELECT
              'sp_KPIListDataCollection_html' AS TableName,
              @LanguageID                    AS LanguageID,
              '-1'                           AS ScreenType,
              @html                          AS html,
              N''                            AS HtmlParadise,
              N''                            AS paradiseJs,
              '2.0-refactor-safe'            AS Version,
              N'Refactor renderer theo 17_RendererHtmlJsSafe: global style, upsert cache ?? 8 c?t, fallback SELECT' AS VersionData
          ) AS src
...
-- Build cache after procedure update. Review before running.
DELETE FROM dbo.tblHtmlScriptCache
WHERE TableName = 'sp_KPIListDataCollection_html';
GO
