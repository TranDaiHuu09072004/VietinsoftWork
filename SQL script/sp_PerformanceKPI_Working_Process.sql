...
Vui lòng làm thủ tục bổ sung nhé ' + isnull(te.FullName,'') , @Now
	...
	BEGIN TRY
		...
		WHEN MATCHED AND Target.ExperiencePonits <> Source.iPoint THEN
			UPDATE SET Target.ExperiencePonits = Source.iPoint
		WHEN NOT MATCHED THEN
			INSERT (EmployeeID, ExperiencePonits, Coin, CreatedDate, PointType)
			VALUES (Source.EmployeeID, Source.iPoint, 0, Source.CreatedDate, Source.PointType);
