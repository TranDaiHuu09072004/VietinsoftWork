/* =============================================================================
   refactor_menu_EmployeeList2_lazyload_20260523.sql
   ---------------------------------------------------------------------------
   Mục đích : Refactor menu MnuHRS501 "Danh sách nhân viên" (ClassName = HR_EmployeeList2)
              sang cơ chế Lazy Load (IntersectionObserver + append) thay vì
              phân trang truyền thống prev/next.
   Tuân thủ : Knowledge/12_CreateMenu.md §Rule 6 + §9 (Grid + Infinite Loop Scroll).
              Knowledge/17_RendererHtmlJsSafe.md (escape T-SQL → JS).
              CLAUDE.md §Bước 6 (idempotent + schema-aware).
   Database : Paradise_Dev (SVRVTS01\SQL2022).
   Tác động : 3 phase
              1. ALTER sp_GetEmployeesList   : bổ sung @Skip/@Take (backward-compat @currentPage).
              2. ALTER HR_EmployeeList2_html : viết lại renderer (IntersectionObserver + append).
              3. Rebuild cache tblHtmlScriptCache cho HR_EmployeeList2_html.
   Idempotent: CREATE OR ALTER (chạy nhiều lần an toàn).
   Khuyến cáo: Backup [Paradise_Dev] trước khi chạy.
   ============================================================================= */
...
PRINT N'================================================================';
PRINT N' REFACTOR MnuHRS501 (HR_EmployeeList2) → Lazy Load';
PRINT N'================================================================';
GO

/* =============================================================================
   PHASE 1 — sp_GetEmployeesList : bổ sung @Skip/@Take
   - Backward-compat @currentPage (nếu @Skip = -1 → dùng @currentPage cũ).
   - Mode mới: @Skip >= 0 → bỏ qua @currentPage, chỉ dùng OFFSET/FETCH theo @Skip/@Take.
   - Trả về thêm: HasMore (bit) tại result set 2 để client biết khi nào dừng.
   ============================================================================= */
PRINT N'PHASE 1: ALTER sp_GetEmployeesList — bổ sung @Skip/@Take...';
GO
...
BEGIN
    ...
    /* ----- Chuẩn hoá keyword (giữ nguyên logic gốc) ----- */
    ...
    /* ----- Chấm điểm match search (giữ nguyên logic) ----- */
...
PRINT N'  [OK] sp_GetEmployeesList đã được refactor.';
GO


/* =============================================================================
   PHASE 2 — HR_EmployeeList2_html : renderer Lazy Load
   - HTML: bỏ pager prev/next + page numbers; thêm <div id="employee-list-sentinel">
   - JS  : IntersectionObserver theo dõi sentinel → fetch chunk tiếp + append
   - Trạng thái: _skip, _take, _hasMore, _isLoading
   - Reset khi filter/search/date thay đổi (clear container + _skip=0)
   - Giữ dual view: card grid (default) + list table (toggle)
   - Bootstrap Icons (bi) — KHÔNG dùng FontAwesome (Knowledge/14_ParadiseStyle.md)
   ============================================================================= */
PRINT N'PHASE 2: ALTER HR_EmployeeList2_html — viết lại renderer lazy load...';
GO
...
PRINT N'  [OK] HR_EmployeeList2_html đã được refactor sang lazy load.';
GO


/* =============================================================================
   PHASE 3 — Rebuild cache tblHtmlScriptCache
   ============================================================================= */
PRINT N'PHASE 3: Rebuild cache tblHtmlScriptCache cho HR_EmployeeList2_html...';
GO
...
BEGIN TRY
    ...
    DELETE FROM dbo.tblHtmlScriptCache
     WHERE TableName = N'HR_EmployeeList2_html';

    -- Sinh cache cho mọi LanguageID đang dùng (sp_GenerateHTMLScript tự loop theo @LanguageID)
    EXEC dbo.sp_GenerateHTMLScript 'HR_EmployeeList2_html', -1;

    COMMIT TRANSACTION;
    PRINT N'  [OK] Cache HR_EmployeeList2_html đã rebuild.';
END TRY
BEGIN CATCH
    IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
    DECLARE @err nvarchar(max) = ERROR_MESSAGE();
    PRINT N'  [LỖI PHASE 3] ' + @err;
    THROW;
END CATCH
GO
...
SELECT
    ...
    o.type_desc,
    o.modify_date,
    DATALENGTH(OBJECT_DEFINITION(o.object_id)) AS DefBytes
FROM sys.objects o
WHERE o.name IN (N'sp_GetEmployeesList', N'HR_EmployeeList2_html')
ORDER BY o.name;

SELECT
    TableName, LanguageID, ScreenType,
...
PRINT N'';
PRINT N'>>> HOÀN TẤT REFACTOR. Logout/login user (LoginID = 3) để test giao diện.';
GO
