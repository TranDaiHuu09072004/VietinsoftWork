-- ============================================================================
-- File      : SQL script/migrate_menu_EmployeeList2_20260524.sql
-- Mục đích  : Di chuyển (migrate) menu MnuHRS501 'Danh sách nhân viên' 
--             và các thủ tục, thiết lập liên quan sang database khác.
--             Giao diện: Lazy Load (IntersectionObserver + infinite scroll).
...
PRINT N'BẮT ĐẦU DI CHUYỂN MENU MNUHRS501 (DANH SÁCH NHÂN VIÊN)...';
GO

-- ============================================================================
-- PHASE 1: Tạo/Cập nhật MEN_Menu cho MnuHRS501
-- ============================================================================
PRINT N'1. Đang tạo/cập nhật MEN_Menu cho MnuHRS501...';
GO
...
BEGIN
    ...
    PRINT N'  [OK] Đã INSERT MEN_Menu MnuHRS501.';
END
ELSE
BEGIN
    UPDATE MEN_Menu
    ...
    PRINT N'  [OK] Đã UPDATE MEN_Menu MnuHRS501.';
END
GO

-- ============================================================================
-- PHASE 2: Thiết lập Phân quyền trong tblSC_Object
-- ============================================================================
...
BEGIN
    ...
    -- Cấp quyền mặc định cho admin group (LoginID = 3) nếu bảng quyền tồn tại
...
PRINT N'  [OK] Đã hoàn thành cấu hình đa ngôn ngữ.';
GO

-- ============================================================================
-- PHASE 4: Cấu hình DataSetting và DataSettingLayout
-- ============================================================================
PRINT N'4. Đang cấu hình tblDataSetting & tblDataSettingLayout...';
GO

IF NOT EXISTS (SELECT 1 FROM tblDataSetting WHERE TableName = 'hr_employeelist2')
BEGIN
    INSERT INTO tblDataSetting (
        ...
        PaintColumns, FormatFontColumns, PaintRows, IsProcessForm, IsShowLayout, 
        IsBatch, LoadDataAfterShow, GroupColumns, FixedColumns, ExportName, 
        CheckLockAttStore, RptTemplate, spAction, DefaultValue, FilterColumn, 
        IsEditForm, ColumnEditSpecial, ColumnsFormatExtend, AllowDelete, 
        PaintCells, ColumnDataType
    )
    VALUES (
        'hr_employeelist2', 'hr_employeelist2', 1, '', '', 
        'col1&0', 'isReadOnlyRow,dtftxxENGColumns', 0, '', 1, 
        '', '', '', 0, 1, 
        0, 1, '', '', '', 
        '', '', '', '', '', 
        0, '', '', 1, 
...
-- Reset & nạp lại các cấu hình layout
DELETE FROM tblDataSettingLayout WHERE TableName = 'hr_employeelist2';

INSERT INTO tblDataSettingLayout (
    TableName, Name, ControlName, NamePa, Type, TypeLayout, ControlType, WidthPercentage
)
VALUES 
('hr_employeelist2', 'root', '', '', 'g', '2', '', 100),
('hr_employeelist2', 'lblcol1', 'col1', 'root', 'i', '2', '_ParadiseHtmlContentControl', 100),
('hr_employeelist2', 'root', '', '', 'g', '6', '', 100),
('hr_employeelist2', 'lblcol1', 'col1', 'root', 'i', '6', '_ParadiseHtmlContentControl', 100);
GO

PRINT N'  [OK] Đã hoàn thành thiết lập layout.';
GO

-- ============================================================================
-- PHASE 5: Tạo các thủ tục lưu trữ
-- ============================================================================
PRINT N'5. Đang tạo các thủ tục lưu trữ liên quan...';
GO
...
-- 5.3. HR_EmployeeList2 (Wrapper)
PRINT N'  - Tạo wrapper HR_EmployeeList2...';
GO
...
BEGIN
    ...
    SELECT html AS col1 FROM tblHtmlScriptCache 
    WHERE TableName = 'HR_EmployeeList2_html' 
...
-- ============================================================================
PRINT N'6. Đang build cache HTML cho HR_EmployeeList2_html...';
GO

DELETE FROM dbo.tblHtmlScriptCache WHERE TableName = N'HR_EmployeeList2_html';
GO

-- Sinh cache cho mọi LanguageID đang dùng
EXEC dbo.sp_GenerateHTMLScript 'HR_EmployeeList2_html', -1;
GO

PRINT N'  [OK] Đã rebuild cache HTML.';
GO
...
PRINT N'DI CHUYỂN MENU MNUHRS501 THÀNH CÔNG!';
GO
