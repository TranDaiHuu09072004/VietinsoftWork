-- ============================================================================
-- File      : SQL script/update_menu_UserLog_html_20260523.sql
-- Mục đích  : Tạo lại và lập trình lại giao diện MnuSCR605 'Nhật ký người dùng' sang phong cách Web.
--             Sử dụng dxDataGrid cuộn vô hạn (Infinite Scroll) kết hợp bộ lọc động và popup diff.
--             Idempotent: IF NOT EXISTS cho tất cả INSERT, CREATE OR ALTER cho procedure.
-- Tác giả   : Antigravity
-- Ngày cập nhật: 2026-05-24 (bổ sung tạo lại menu sau khi bị xóa)
...
<div id="ezLogRoot" class="ezl-page">
    ...
    <div class="ezl-title-row">
        <h2 class="ezl-title"><i class="bi bi-clock-history me-2"></i></h2>
    </div>
    ...
    <div class="ezl-filter-bar">
        ...
        <div class="ezl-filter-item ezl-align-end">
...
(() => {
    ...
    $("#ezLogRoot .ezl-title").html(''<i class="bi bi-clock-history me-2"></i>'' + curLabels.title);
    ...
    function getGridHeight() {
        const titleEl = document.querySelector(".ezl-title-row");
        const filterEl = document.querySelector(".ezl-filter-bar");
        const tHeight = titleEl ? titleEl.offsetHeight : 40;
        const fHeight = filterEl ? filterEl.offsetHeight : 80;
        return window.innerHeight - tHeight - fHeight - 110;
    }
