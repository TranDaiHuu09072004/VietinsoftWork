-- ===========================================================================
-- Fix: Renderer chuẩn 6-layer (17_RendererHtmlJsSafe) — dùng '
' thay cho ' trong JS regex
-- Chạy file này trong SSMS trên DB Paradise_Dev
-- ===========================================================================
SET NOCOUNT ON;
SET XACT_ABORT ON;
GO
...
BEGIN
    ...
    -- ================================================================
    -- Layer 3: JS-escaped (Rule 3: escape \ trước, rồi ")
    -- ================================================================
    ...
    -- ================================================================
    -- Layer 4: Build @html — dùng SET @html = @html + N'...' từng phần
    -- ================================================================
    ...
    -- ================================================================
    -- JavaScript (Rule 2: dùng '
' thay cho ' trong regex)
    -- ================================================================
    SET @html = @html + N'<script>(function(){';
    SET @html = @html + N'var _currentKeyword="",_pageCache={},_fromDate=null,_toDate=null;';
    ...
    SET @html = @html + N'function loadDashboardStats(){';
    SET @html = @html + N'AjaxHPAParadise({data:{name:"sp_LeadTrackingForMarketing_GetStats",param:["LoginID",window.UserID||window.LoginID,"LanguageID",window.LanguageID]},';
    SET @html = @html + N'success:function(res){';
    SET @html = @html + N'var json=typeof res==="string"?JSON.parse(res):res;';
    SET @html = @html + N'var stats=(json&&json.data&&json.data[0]&&json.data[0][0])||{};';
    SET @html = @html + N'document.getElementById("valTotalLeads").textContent=(stats.TotalLeads||0).toLocaleString();';
    SET @html = @html + N'document.getElementById("valLeadsThisMonth").textContent=(stats.NewLeadsThisMonth||0).toLocaleString();';
    SET @html = @html + N'document.getElementById("valLeadsThisWeek").textContent=(stats.NewLeadsThisWeek||0).toLocaleString();';
    SET @html = @html + N'}});}';

    -- CustomStore (Rule 6)
    SET @html = @html + N'var dataStore=new DevExpress.data.CustomStore({key:"CRM_CustomerID",load:function(lo){';
    SET @html = @html + N'var d=$.Deferred();var p=[];';
    ...
    SET @html = @html + N'var sort=lo.sort?lo.sort.map(function(s){return s.selector+(s.desc?" DESC":" ASC");}).join(","):"STT";';
    SET @html = @html + N'p.push("@Sort","ORDER BY "+sort);';
    SET @html = @html + N'if(_currentKeyword){p.push("@SearchValue",_currentKeyword);p.push("@ColumnSearch","FullName,PhoneNumber,Email");}';
    SET @html = @html + N'if(lo.filter){var hf=false;try{hf=JSON.stringify(lo.filter,function(k,v){return typeof v==="function"?"F":v;}).indexOf("F")>=0;}catch(e){}if(!hf)p.push("@Filters",createConditionQuery(lo.filter));}';
    SET @html = @html + N'if(_fromDate)p.push("@FromDate",_fromDate);if(_toDate)p.push("@ToDate",_toDate);';
    SET @html = @html + N'AjaxHPAParadise({data:{name:"sp_LoadGridUsingAPI",param:p},';
    SET @html = @html + N'success:function(res){';
    SET @html = @html + N'var json=typeof res==="string"?JSON.parse(res):res;';
    SET @html = @html + N'var results=Array.isArray(json&&json.data&&json.data[0])?json.data[0]:[];';
    SET @html = @html + N'var result={data:results};';
    SET @html = @html + N'if(lo.requireTotalCount)result.totalCount=(json&&json.data&&json.data[1]&&json.data[1][0]&&json.data[1][0].TotalCount)||0;';
    SET @html = @html + N'd.resolve(result);';
    SET @html = @html + N'},error:function(){d.reject(ERR_MSG);}});';
    SET @html = @html + N'return d.promise();}});';

    -- dxDataGrid (Rule 6 — infinite scroll)
    SET @html = @html + N'var gridEl=document.getElementById("GridLeadTracking");';
    SET @html = @html + N'var grid=$(gridEl).dxDataGrid({';
    SET @html = @html + N'dataSource:dataStore,';
    SET @html = @html + N'remoteOperations:{paging:true,filtering:true,sorting:true,searching:true},';
    ...
    SET @html = @html + N'onOptionChanged:function(e){if(e.name==="searchPanel"&&e.fullName==="searchPanel.text"){_currentKeyword=(e.value||"").trim();_pageCache={};}}';
    SET @html = @html + N'}).dxDataGrid("instance");';

    -- Filter handlers
    SET @html = @html + N'function applyFilter(){';
    SET @html = @html + N'_fromDate=document.getElementById("filterFromDate").value||null;';
    SET @html = @html + N'_toDate=document.getElementById("filterToDate").value||null;';
    SET @html = @html + N'_pageCache={};grid.refresh();loadDashboardStats();}';
    SET @html = @html + N'function resetFilter(){';
    SET @html = @html + N'document.getElementById("filterFromDate").value="";';
    SET @html = @html + N'document.getElementById("filterToDate").value="";';
    SET @html = @html + N'_fromDate=null;_toDate=null;_pageCache={};grid.refresh();loadDashboardStats();}';
    ...
    SET @html = @html + N'loadDashboardStats();';
    SET @html = @html + N'})();</script>';

    -- ================================================================
    -- Layer 5: MERGE cache (8 cột)
    -- ================================================================
...
PRINT '  [+] Created sp_LeadTrackingForMarketing_html (6-layer)';
...
PRINT '=== DONE: Chạy logout/login để thấy menu MnuKPI448 ===';
