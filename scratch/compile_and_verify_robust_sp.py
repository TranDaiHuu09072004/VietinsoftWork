...
try:
    print("Compiling robust sp_Level_AutoUpdateHistory...")
    cursor.execute(sp_ddl)
    conn.commit()
    print("Compilation successful.")
    
    # Let's perform a test run:
    # First, print current level history
    print("\nLevel history before run:")
    cursor.execute("SELECT EmployeeID, LevelID, EffectiveDate, Remark FROM tblLevelIDHistory")
    for r in cursor.fetchall():
        print(f"Emp: {r[0]:<5} | LevelID: {r[1]:<3} | Date: {r[2]} | Remark: {r[3]}")
        
    # Execute the procedure
    print("\nExecuting sp_Level_AutoUpdateHistory (Run 1)...")
    cursor.execute("EXEC dbo.sp_Level_AutoUpdateHistory")
    conn.commit()
    print("Run 1 completed.")
    
    # Print current level history again
    print("\nLevel history after Run 1:")
    cursor.execute("SELECT EmployeeID, LevelID, EffectiveDate, Remark FROM tblLevelIDHistory")
    for r in cursor.fetchall():
        print(f"Emp: {r[0]:<5} | LevelID: {r[1]:<3} | Date: {r[2]} | Remark: {r[3]}")
        
    # Execute the procedure again (idempotency check / multiple run check)
    print("\nExecuting sp_Level_AutoUpdateHistory (Run 2 - Idempotency)...")
    cursor.execute("EXEC dbo.sp_Level_AutoUpdateHistory")
    conn.commit()
    print("Run 2 completed.")
    
    # Print current level history again
    print("\nLevel history after Run 2:")
    cursor.execute("SELECT EmployeeID, LevelID, EffectiveDate, Remark FROM tblLevelIDHistory")
    for r in cursor.fetchall():
        print(f"Emp: {r[0]:<5} | LevelID: {r[1]:<3} | Date: {r[2]} | Remark: {r[3]}")
