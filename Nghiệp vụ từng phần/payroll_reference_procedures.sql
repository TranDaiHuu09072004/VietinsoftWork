-- DANH SÁCH CÁC STORED PROCEDURE TÍNH LƯƠNG
-- Tài liệu tham khảo theo yêu cầu của user

----------------------------------------------------
-- PROCEDURE: SALCAL_ADJUSTMENT_FINISHED
----------------------------------------------------
...
----------------------------------------------------
-- PROCEDURE: SALCAL_ALLOWANCE_FINISHED
----------------------------------------------------
...
----------------------------------------------------
-- PROCEDURE: SALCAL_INSURANCE
----------------------------------------------------
CREATE PROCEDURE [dbo].[SALCAL_INSURANCE]
...
DECLARE
			...
			,@SalStop		datetime
			,@InsDate		datetime
			,@InsDayParam		varchar(3)
			,@MaternityLeaveCode 	varchar(20)

			,@HI_EMP_PERCENT	FLOAT
			,@SI_EMP_PERCENT 	FLOAT
			,@HI_COM_PERCENT	FLOAT
			,@SI_COM_PERCENT	FLOAT
			,@UI_EMP_PERCENT	float
			,@UI_COM_PERCENT	float
			...
			,@UI_LIMIT_MAX		money
			,@UI_LIMIT_MIN		money
			,@INS_UNPAIDLEAVEFULL	bit

	DECLARE @TableVarActiveEmp TABLE(
	...
	DECLARE	@TableVarInsurance TABLE(
				EmployeeID			varchar(20)
				,JoinDate			datetime
				,EmployeeStatusID	int
				,TerminateDate		datetime
				...
				,StatusResign		bit
				,StatusFullUnPaidLeave	bit
				,StatusMaternity	bit
				,StatusIncreaseSal	bit
				,MaternityDateMin	datetime
				,MaternityDateMax	datetime

				,SalaryHistoryID	int
				,ContractSalary		money
				,SkillID			int
				,PositionID			int
				,ContractSalDate	datetime
				...
				,UIComp	money
				,TotalInsComp		money
				,SalaryNameID		VARCHAR(20)
				)	

	DECLARE	@TalbeVarMaternity	TABLE(
				EmployeeID			varchar(20)
...
BEGIN
	...
	--Get the InsDate
	----------------------------------------------------
	SET 	@InsDayParam = (SELECT [Value] FROM tblParameter WHERE Code = 'INSURANCE_DATE')
	SET	@InsDate = Cast(Cast(@Year as varchar) + '-' + Cast(@Month as varchar) + '-' + @InsDayParam as datetime)
	SET 	@MaternityLeaveCode = (SELECT [Value] FROM tblParameter WHERE Code = 'LEAVE_CODE_MATERNITY')

select @HI_EMP_PERCENT = HI_EmpPercent,@SI_EMP_PERCENT  = SI_EmpPercent, @UI_EMP_PERCENT = UI_EmpPercent,
		@HI_COM_PERCENT = HI_CompPercent, @SI_COM_PERCENT = SI_CompPercent, @UI_COM_PERCENT = UI_CompPercent
 from tblSIPercentageHistory where SI_ID = (select max(SI_ID) from tblSIPercentageHistory where SI_EffectiveDate < @SalStop)




select @HI_ISLIMIT_MAX =1,@SI_ISLIMIT_MAX = 1,@HI_LIMIT_MAX = a.HI_Salary,@SI_LIMIT_MAX = a.SI_Salary, @UI_LIMIT_MAX = a.UI_Salary
 from tblSI_CeilSalary  a WHERE a.EffectiveDate = (
 SELECT Max(sie.EffectiveDate) EffectiveDate  FROM tblSI_CeilSalary sie)

	SET 	@INS_UNPAIDLEAVEFULL = (SELECT Cast(Value as bit) FROM tblParameter WHERE Code = 'INS_UNPAIDLEAVEFULL')
	


		INSERT INTO @TableVarActiveEmp(
			EmployeeID
			,JoinDate
			,EmployeeStatusID
			,TerminateDate
		...
		SELECT	e.EmployeeID
				...
				,e.EmployeeStatusID
				,e.TerminateDate
		FROM	dbo.fn_vtblEmployeeList_Bydate(Getdate(),'-1',null) e inner join #TempTableSalDetail t on e.EmployeeID = t.EmployeeID


	
	INSERT INTO @TableVarInsurance(
		EmployeeID
		,JoinDate
		,EmployeeStatusID
		,TerminateDate
		,StatusResign
		,StatusFullUnPaidLeave
		,StatusMaternity
		,StatusIncreaseSal
		,IsCalculate
		)
		(SELECT	e.EmployeeID
				...
				,e.EmployeeStatusID
				,e.TerminateDate
				,0 --default of resign status
				,0 --default of full unpaid leave status
				,0 --default of maternity leave status
				,0 --default of increase sal status
				,1
		FROM	dbo.fn_vtblEmployeeList_Bydate(Getdate(),'-1',null) e
		WHERE e.EmployeeID IN (SELECT EmployeeID FROM @TableVarActiveEmp)
			AND E.employeeID not in (select EmployeeID from tblEmployee where positionID in (select PositionID from tblPosition where SalaryCalPerDay = 1))
		)
	
	--------------------------------------------------
	-- Deal with Maternity leave
	--------------------------------------------------
	--Temple table: Schedule
	...
	INSERT INTO @TmpSchedule(EmployeeID,ScheduleDate,ShiftID,HolidayStatus,LeaveStatus)
	SELECT 	sch.EmployeeID
		,ScheduleDate
		,ShiftID
		,HolidayStatus
		,LeaveStatus
	FROM	tblWSchedule sch
	left join (select EmployeeID, LeaveDate, Max(LeaveStatus) LeaveStatus from tblLvHistory where LeaveDate <= @SalStop
			AND LeaveDate >= @SalStart AND EmployeeID IN (SELECT EmployeeID FROM @TableVarActiveEmp) group by EmployeeID, LeaveDate
			) lv on sch.EmployeeID = lv.EmployeeID and DATEDIFF(day,sch.ScheduleDate,lv.LeaveDate) = 0
	WHERE	ScheduleDate <= @SalStop
			AND ScheduleDate >= @SalStart
			AND sch.EmployeeID IN (SELECT EmployeeID FROM @TableVarActiveEmp)
	...
	INSERT INTO @TmpLeave(EmployeeID, LeaveDate,LeaveCode,LeaveStatus)
	SELECT	EmployeeID
		,LeaveDate
		,LeaveCode
		,LeaveStatus
	FROM	tblLvHistory
	WHERE	LeaveDate <= @SalStop
			AND LeaveDate >= @SalStart
			AND EmployeeID IN (SELECT EmployeeID FROM @TableVarActiveEmp)

	UPDATE @TmpLeave
	SET	PaidRate = lt.PaidRate
	FROM	@TmpLeave l, tblLeaveType lt
	WHERE	l.LeaveCode = lt.LeaveCode
	


	--Temple table Maternity leave
	INSERT INTO @TalbeVarMaternity(
			EmployeeID
			,ScheduleStart
			,ScheduleEnd
			,MaternityDateMin
			,MaternityDateMax)
	(SELECT tl.EmployeeID
			,MIN(ts.ScheduleDate)
			,MAX(ts.ScheduleDate)
			,MIN(tl.LeaveDate)
			,MAX(tl.LeaveDate)
	FROM	@TmpLeave tl
			INNER JOIN @TmpSchedule ts ON tl.EmployeeID = ts.EmployeeID
	WHERE	tl.LeaveCode = @MaternityLeaveCode
			AND ts.HolidayStatus = 0
	GROUP BY tl.EmployeeID
	)

	UPDATE	@TalbeVarMaternity
	SET		CalculateIns = (CASE
						WHEN MaternityDateMin <= @InsDate AND MaternityDateMax = ScheduleEnd THEN 0
						WHEN MaternityDateMin = ScheduleStart AND MaternityDateMax >= @InsDate THEN 0
						WHEN MaternityDateMin = ScheduleStart AND MaternityDateMax = ScheduleEnd THEN 0
						ELSE 1
					END)
	...
	------------------------------------------------
	UPDATE	@TableVarInsurance
	SET 	SalaryHistoryID = (SELECT TOP 1 SalaryHistoryID FROM tblSalaryHistory s
						WHERE t.EmployeeID = s.EmployeeID
							AND s.[Date] <= @InsDate
						ORDER BY s.[Date] DESC)
	FROM	@TableVarInsurance t

	UPDATE 	@TableVarInsurance
	SET	ContractSalary		= s.Salary
		,ContractSalDate	= s.[Date]
	FROM	tblSalaryHistory s, @TableVarInsurance t
	WHERE	s.EmployeeID = t.EmployeeID
		AND s.SalaryHistoryID = t.SalaryHistoryID
UPDATE 	@TableVarInsurance SET	ContractSalary		= 0 WHERE ContractSalary IS NULL

	--THOONG TIN DONG BAO HIEM DUA TREN HOP DONG LAO DONG HIEN TAI
	UPDATE	t
	SET		IsHIEmp = ISNULL(tmp.Emp_HealthInsurance,0)
			,IsSIEmp = ISNULL(tmp.Emp_SocialInsurance,0)
			,IsUIEmp = isnull(tmp.Emp_UnEmployedInsurance,0)
			,IsHIComp = ISNULL(tmp.Com_HealthInsurance,0)
			,IsSIComp = ISNULL(tmp.Com_SocialInsurance,0)
			,IsUIComp = isnull(tmp.Com_UnEmployedInsurance,0)
	FROM	@TableVarInsurance t inner join tblEmpInsuranceMonthly tmp on t.EmployeeID = tmp.EmployeeID and tmp.Month= @Month and tmp.Year =@Year
	
	--select * from @TableVarInsurance where EmployeeID='00007'
	UPDATE @TableVarInsurance
	SET IsHIEmp = isnull(IsHIEmp,0)
		,IsSIEmp = isnull(IsSIEmp,0)
		,IsHIComp = isnull(IsHIComp,0)
		,IsSIComp = isnull(IsSIComp,0)
		,IsUIEmp = isnull(IsUIEmp,0)
		,IsUIComp = isnull(IsUIComp,0)


	----------------------------------------------------
	--Determine more information about employee status
	----------------------------------------------------
	--Resign
	UPDATE @TableVarInsurance
	SET		StatusResign = 1
	WHERE	EmployeeStatusID = 20 AND DATEDIFF(dd,@SalStop,TerminateDate) <= 0

	--Full unpaid leave
	UPDATE	@TableVarInsurance
	SET	StatusFullUnPaidLeave = 1
	WHERE	EmployeeID NOT IN (SELECT DISTINCT sch.EmployeeID
					FROM @TmpSchedule sch
						LEFT OUTER JOIN @TmpLeave l ON sch.EmployeeID = l.EmployeeID AND DATEDIFF(dd,sch.ScheduleDate,l.LeaveDate)=0
					WHERE (sch.LeaveStatus IN (0,1,2) OR (sch.LeaveStatus = 3 AND l.PaidRate >0))
							AND HolidayStatus =0
					)
	--Maternity leave
	UPDATE @TableVarInsurance
	SET		StatusMaternity = 1
	WHERE	EmployeeID IN (SELECT EmployeeID FROM @TalbeVarMaternity)


	
	----------------------------------------------------
	-- Terminate Insurance income
	----------------------------------------------------
	UPDATE @TableVarInsurance
	SET 	SIIncome = (CASE
					WHEN @SI_ISLIMIT_MAX = 1 AND ContractSalary > @SI_LIMIT_MAX THEN @SI_LIMIT_MAX
					WHEN @SI_ISLIMIT_MIN = 1 AND COntractSalary < @SI_LIMIT_MIN THEN @SI_LIMIT_MIN
					ELSE ContractSalary
				END)
		,HIIncome = (CASE
					WHEN @HI_ISLIMIT_MAX = 1 AND ContractSalary > @HI_LIMIT_MAX THEN @HI_LIMIT_MAX
					WHEN @HI_ISLIMIT_MIN = 1 AND ContractSalary < @HI_LIMIT_MIN THEN @HI_LIMIT_MIN
					ELSE ContractSalary
				END)
		,UIIncome = (CASE
			WHEN ContractSalary > @UI_LIMIT_MAX THEN @UI_LIMIT_MAX
			WHEN ContractSalary < @UI_LIMIT_MIN THEN @UI_LIMIT_MIN
			ELSE ContractSalary
		END)
	------------------------------------------------------
	--Calculate Insurance
	------------------------------------------------------	
	UPDATE	@TableVarInsurance
	SET		SIEmp = (@SI_EMP_PERCENT/100.00) * SIIncome * IsCalculate * IsSIEmp
			,HIEmp	= (@HI_EMP_PERCENT/100.00) * HIIncome * IsCalculate * IsHIEmp
			,UIEmp  = (@UI_EMP_PERCENT/100.00) * UIIncome * IsCalculate *IsUIEmp
			,SIComp	= (@SI_COM_PERCENT/100.00) * SIIncome * IsCalculate * IsSIComp
			,HIComp	= (@HI_COM_PERCENT/100.00) * HIIncome * IsCalculate * IsHIComp
			,UIComp = (@UI_COM_PERCENT/100.00) * UIIncome * IsCalculate * IsUIComp
	UPDATE	@TableVarInsurance set SIComp = ((@SI_COM_PERCENT+@SI_EMP_PERCENT)/100.00) * SIIncome * IsCalculate * IsSIComp where IsSIEmp = 0
	UPDATE	@TableVarInsurance set HIComp = ((@HI_COM_PERCENT+@HI_EMP_PERCENT)/100.00) * HIIncome * IsCalculate * IsHIComp where IsHIEmp = 0
	UPDATE	@TableVarInsurance set UIComp = ((@UI_COM_PERCENT+@UI_EMP_PERCENT)/100.00) * UIIncome * IsCalculate * IsUIComp where IsUIEmp = 0

	UPDATE @TableVarInsurance
	SET		TotalInsEmp = SIEmp + HIEmp + UIEmp
			,TotalInsComp = SIComp + HIComp + UIComp
			
			
	-------------------------------------------------------------
	-- INSERT Insurance data into table
	-------------------------------------------------------------
	DELETE FROM tblSal_Insurance
	WHERE [Month] = @Month
			AND [Year] = @Year
			AND EmployeeID IN (SELECT EmployeeID FROM @TableVarActiveEmp)

	INSERT INTO tblSal_Insurance(
			EmployeeID
			,[Month]
			,[Year]
			,SalaryHistoryID
	...
	(SELECT
			...
			,UIComp
			,TotalInsComp
			,ISNULL(HIEmp,0) + ISNULL(SIEmp,0) + ISNULL(HIComp,0) + ISNULL(SIComp,0) + isnull(UIEmp,0) + isnull(UIComp,0)
			
	FROM @TableVarInsurance
	)
	
update #TempTableSalDetail set InsAmt = tmp.insAmt from #TempTableSalDetail sal,(
SELECT EmployeeID,ISNULL(HIEmp,0) + ISNULL(SIEmp,0) + ISNULL(UIEmp,0)  insAmt FROM @TableVarInsurance
...
----------------------------------------------------
-- PROCEDURE: SALCAL_MAIN
----------------------------------------------------
--exec SALCAL_MAIN 2,2026,3,0,'029',0
CREATE PROCEDURE [dbo].[SALCAL_MAIN]
(
	@Month int,
	@Year int,
	@LoginID int,
...
exec SALCAL_ADD_COLUMN_INTO_TMP_TABLE @StopUPDATE output, @Month,@Year, @FromDate ,@ToDate ,@LoginID,@PeriodID,@EmployeeID
 ...
 select te.EmployeeID,te.DivisionID,te.DepartmentID,te.SectionID,te.GroupID,te.EmployeeTypeID,te.PositionID,te.EmployeeStatusID,te.Sex
 ,case when te.HireDate > @fromDate then cast(1 as bit) else 0 end as NewStaff
 ,CAST(0 as bit) as TerminatedStaff
 ,HireDate,RenewHireDate,CAST(NULL as datetime) TerminateDate,ProbationEndDate
 into #tblEmployeeIDList
 from dbo.fn_vtblEmployeeList_Bydate(@ToDateTruncate,@EmployeeID,@LoginID) te
 inner join tblDivision div on te.DivisionID = div.DivisionID
 where
 -- khoa roi thi khong tinh luong nua
 -- nếu khóa thì ko tính lương thường chỉ tính retro thôi
  te.EmployeeID not in (select l.EmployeeID from tblSal_Lock l where @CalculateRetro = 0 and l.Month = @Month and l.Year = @Year)
 --and isnull(te.IsWorker,1) = 1
 --and exists(SELECT 1 FROM tblAtt_LockMonth sal where te.EmployeeID = sal.EmployeeID and sal.Month = @Month and sal.Year = @Year )
	...
	begin
		-- mức lương cũ
		insert into #tblSalDetail(EmployeeID,SalaryHistoryID,FromDate,ToDate,BasicSalary,BasicSalaryOrg,SalCalRuleID,LatestSalEntry,BaseSalRegionalID,CurrencyCode,IsNet,PayrollTypeCode,WorkingHoursPerDay)
		select sh.EmployeeID,sh.SalaryHistoryID,case when sh.Date < @FromDate then @FromDate else sh.Date end ,@ToDate,sh.Salary,sh.Salary,sh.SalCalRuleID,1,sh.BaseSalRegionalID
		,sh.CurrencyCode,sh.IsNet,sh.PayrollTypeCode,ISNULL(nullif(sh.WorkingHoursPerDay,0),8)
		...
		where exists(select 1 from #tblEmployeeIDList te where sh.EmployeeID = te.EmployeeID and sh.Date >= te.HireDate)
	end
	
	
	-- muc luong phat sinh moi trong thang
	insert into #tblSalDetail(EmployeeID,SalaryHistoryID,FromDate,ToDate,BasicSalary,BasicSalaryOrg,SalCalRuleID,LatestSalEntry,BaseSalRegionalID
	,CurrencyCode,IsNet,PayrollTypeCode,WorkingHoursPerDay)
	select sh.EmployeeID,sh.SalaryHistoryID,case when sh.Date < @FromDate then @FromDate else sh.Date end ,@ToDate,sh.Salary,sh.Salary,sh.SalCalRuleID,1,sh.BaseSalRegionalID
	,sh.CurrencyCode,sh.IsNet,sh.PayrollTypeCode,ISNULL(nullif(sh.WorkingHoursPerDay,0),8)
	from
	 tblSalaryHistory sh
...
exec SALCAL_CUSTOMIZE_TADATA @StopUPDATE output, @Month,@Year, @FromDate ,@ToDate ,@LoginID,@PeriodID,@EmployeeID
	...
	,SUM(ISNULL(AttDays,0)+ ISNULL(PaidLeaves,0)) as TotalPaidDays
	...
	select lv.EmployeeID,lv.LeaveCode,lv.LvAmount,lv.LeaveDate, lt.PaidRate,lv.LeaveStatus,lv.NotAllowDiligence,ISNULL(lt.LeaveCategory,0) as LeaveCategory
	into #tblLvHistory
	from tblLvHistory lv
	inner join tblLeaveType lt on lv.LeaveCode = lt.LeaveCode
	and lv.EmployeeID in(select te.EmployeeID from #tblEmployeeIDList te
	except
	select c.EmployeeID from #tblCustomAttendanceData c -- except để giảm thời gian thực thi bớt di
	)
	...
	inner join #Tadata ta on lv.EmployeeID = ta.EmployeeID and lv.LeaveDate = ta.Attdate
	and  ta.CutSI = 1

	--LongKa : Xin đi trễ về sớm nửa tiếng không bị trừ lương
	UPDATE #tblLvHistory SET PaidRate = CASE WHEN LvAmount > 0.5 THEN 0 ELSE 100 END where LeaveCode = 'IO'
	...
	from #tblCustomAttendanceData where '+LeaveCode +' <>0' from tblLeaveType lt
	where lt.LeaveCode in(select column_Name from INFORMATION_SCHEMA.COLUMNS c where c.TABLE_NAME = 'tblCustomAttendanceData')
	exec(@CustomLeaveCodeInsert)

	insert into #tblLvHistory(EmployeeID,LeaveCode,LvAmount,LeaveDate,PaidRate,LeaveStatus,NotAllowDiligence,LeaveCategory)
	select lv.EmployeeID,lv.LeaveCode,lv.lvAmountdays*8,@ToDateTruncate as  LeaveDate, lt.PaidRate,3 as LeaveStatus,0 as NotAllowDiligence,ISNULL(lt.LeaveCategory,0) as LeaveCategory
	...
	if not exists (select 1 from tblLeaveType where LeaveCode in (select Value from tblParameter where Code = 'NO_TA_LEAVE_CODE'))
		update tblParameter set Value = (select top 1 LeaveCode from tblLeaveType where PaidRate = 0) where Code = 'NO_TA_LEAVE_CODE'

	insert into #tblLvHistory(EmployeeID,LeaveCode,LvAmount,LeaveDate,PaidRate,LeaveStatus,LeaveCategory)
	select EmployeeID,(select Value from tblParameter where Code = 'NO_TA_LEAVE_CODE')
	...
	UPDATE TA set Lvamount = isnull(Lvamount,0) + (Std_Hour_PerDays - (isnull(WorkingTime,0) + isnull(Lvamount,0))), UnpaidAmount_Des = isnull(UnpaidAmount_Des,0)  + (Std_Hour_PerDays - (isnull(WorkingTime,0) + isnull(Lvamount,0)))
	from #Tadata ta
	where ta.HolidayStatus = 0 and isnull(WorkingTime,0) + isnull(Lvamount,0) < Std_Hour_PerDays
	...
	from #Tadata ta
	inner join tblEmployeeStatus es on ta.EmployeeStatusID = es.EmployeeStatusID and es.LeaveCodeForCutSI in(select LeaveCode from tblLeaveType)
	where ta.CutSI= 1 and es.LeaveCodeForCutSI is not null and ta.HolidayStatus = 0
	) lv group  by EmployeeID,LeaveCode
	...
	from #tblEmployeeIDList e where not exists(select 1 from tblSal_AttDataSumary_ForReport s where e.EmployeeID = s.EmployeeID and s.Month= @Month and s.Year = @Year and s.PeriodID= @PeriodID)

	-- phải chơi hết ko thôi mấy cái update rồi giờ ko update nữa là khộ
	select distinct LeaveCode into #LeaveCode from tblLeaveType
	if @CalculateRetro = 0
	while exists(select 1 from #LeaveCode)
	begin
		select @currLeaveCode = LeaveCode from #LeaveCode
		...
		exec( @altertblSal_AttDataSumary_ForReport)
		delete #LeaveCode where LeaveCode=  @currLeaveCode
		delete #LeaveTMpDAta where LeaveCode=  @currLeaveCode
	end
	...
	delete #tblLvHistory where LeaveCategory =0

	-- cap nhat tong luong
	UPDATE sal set TotalSalary = c.Totalsalary from #tblSalDetail sal
	cross apply dbo.fn_CurrentSalaryByDate(@ToDate, @LoginID) c where sal.SalaryHistoryID = c.SalaryHistoryID
	
	--UPDATE sal set TotalSalary = c.Totalsalary from #tblSalDetail sal
	--inner join dbo.fn_CurrentSalaryByDate(@FromDate) c on sal.SalaryHistoryID = c.SalaryHistoryID
...
------------------------------Calculate salary per day------------------------------------------
	...
	update sal set OTSalary =  fnTO.OTSalary from
	(select distinct Fromdate from #tblSalDetail) f
	cross apply (select * from  dbo.fn_CurrentOTSalary_byDate(f.FromDate,@LoginID) fnTO ) fnTO
	inner join #tblSalDetail sal
		on sal.EmployeeID = fnTO.EmployeeID and sal.SalaryHistoryID = fnTO.SalaryHistoryID
	...
	UPDATE #tblSalDetail SET OTSalary = OTSalary - BasicSalaryOrg + BasicSalary where BasicSalaryOrg is not null and BasicSalaryOrg <> BasicSalary
	...
	UPDATE #tblSalDetail SET SalaryPerDay =
	 case when isnull(IsDaily,0)= 1 then BasicSalary
			when isnull(IsHourly,0)=1 then BasicSalary*8.0
			else
			 BasicSalary/STD_WD end
	 ,SalaryPerDayOT =
	 case when isnull(IsDaily,0)= 1 then BasicSalary
		 when isnull(IsHourly,0)= 0 then OTSalary/24.0 end -- fix for demo
	from #tblSalDetail sal
	inner join tblSalaryCalculationRule sc on sal.SalCalRuleID= sc.SalCalRuleID
...
------------------------------Calculate salary per hour------------------------------------------
	...
	update #tblSalDetail set SalaryPerHour =case when isnull(IsHourly,0) = 1 then BasicSalary else SalaryPerDay/8.0 end
	, SalaryPerHourOT =case when isnull(IsHourly,0) = 1 then BasicSalary else SalaryPerDayOT/sal.WorkingHoursPerDay end
	from #tblSalDetail sal
	inner join tblSalaryCalculationRule sc on sal.SalCalRuleID= sc.SalCalRuleID
...
INSERT INTO #tblSal_PaidLeave_Detail_des(EmployeeID,LeaveCode,Month,Year,SalaryHistoryID,LeaveDays,LeaveHour,AmountDeduct,AmountPaid,PeriodID)
(SELECT EmployeeID
	,LeaveCode
	,@Month
	,@Year
	...
	,SUM(LeaveDays)
	,SUM(LeaveHours)
	,ROUND(SUM(AmountDeduct),@ROUND_SALARY_UNIT)
	,ROUND(SUM(AmountPaid),@ROUND_SALARY_UNIT)
	,@PeriodID
	FROM #TableVarLeaveAmount
	group by EmployeeID,LeaveCode,SalaryHistoryID
)
...
INSERT INTO #tblSal_PaidLeave_des(
	EmployeeID
	,LeaveCode
	,[Month]
	,[Year]
...
(SELECT EmployeeID
	...
	,SUM(ISNULL(LeaveDays,0))
	,SUM(ISNULL(LeaveHours,0))
	,ROUND(SUM(ISNULL(AmountDeduct,0)),@ROUND_SALARY_UNIT)
	,ROUND(SUM(ISNULL(AmountPaid,0)),@ROUND_SALARY_UNIT)
	,''
	,0
	,@PeriodID
	FROM #TableVarLeaveAmount
...
update #tblSalDetail  set
	DaysOfSalEntry = ISNULL(ca.AttDays,0)+ISNULL(ca.PaidLeaves,0)
	,STDPerSalaryHistoryId= case when CA.STD_PerHistoryID > sal.STD_WD then sal.STD_WD else ca.STD_PerHistoryID end
from #tblSalDetail sal
	cross apply(select * from #tblSal_AttendanceData_PerHistory ta where sal.EmployeeID = ta.EmployeeID and ta.SalaryHistoryID = ISNULL(sal.ProbationSalaryHistoryID,sal.SalaryHistoryID)
) ca
...
update sal set ActualMonthlyBasic= ActualMonthlyBasic - (isnull(TotalNonWorkingDays,0)*SalaryPerDay )
from #tblSalDetail sal
...
insert into #tblAllowance(EmployeeID,AllowanceID,AllowanceRuleID,AllowanceCode,SalaryHistoryID,FromDate,ToDate,DefaultAmount,TakeHome,SalCalRuleID,TotalPaidDays,IsMutilCurrencyCode,TaxFreeMaxAmount
,LatestSalEntry)
...
--LongKa : tháng 1/2025 bị lỗi, a Chiến không nhập thưởng Tết vào, chuyển khoản 1 tháng lương luôn nên bị không tính thuế
if @Year*12+@Month = 2025*12+1
begin
	UPDATE #AdjustmentSum SET UntaxableAmount = AdjustmentAmount where IncomeID = 4
end
...
UPDATE #tblAllowance SET STD_WD = CASE WHEN A.AllowanceRuleID IN (3,7) THEN @FIXEDWORKINGDAY when AllowanceRuleID in(4,8) then sal.STD_WD_Schedule else sal.STD_WD END
from #tblAllowance a
inner join #tblSalDetail sal on a.EmployeeID = sal.EmployeeID

update a set TotalPaidDays = sal.STD_WD - lv.lvAmount
	 from #tblSalDetail sal
	inner join tblSalaryCalculationRule sc on sal.SalCalRuleID = sc.SalCalRuleID
	inner join #tblAllowance  a on sal.EmployeeID = a.EmployeeID
	inner join (select EmployeeID, ISNULL(SUM(lv.LvAmount/8.0),0) as lvAmount from #tblLvHistory lv inner join tblLeaveType lt on lv.LeaveCode = lt.LeaveCode and isnull(lt.PaidRate,0) = 0 and lv.LeaveDate between @FromDate and @ToDate group by lv.EmployeeID) lv
	on a.EmployeeID = lv.EmployeeID
	where a.AllowanceRuleID = 17 and sc.IsSTDMinusUnpaidLeave = 1
	-- tinh theo luong co ban
	
--chia cong chuan * cong thuc te
update #tblAllowance set TotalPaidDays = sal.AttDays
from #tblAllowance al
...
UPDATE #tblAllowance SET TotalPaidDays = ISNULL(att.AttDays,0) + ISNULL(att.ProbationHours,0)/8.0
from #tblAllowance a
inner join #tblSal_AttendanceData att on a.EmployeeID = att.EmployeeID
and AllowanceRuleID in (2,10,19)


UPDATE #tblAllowance SET TotalPaidDays = ISNULL(a.TotalPaidDays,0) + ISNULL(lv.LvAmount,0)
from #tblAllowance a inner join (select EmployeeID, ISNULL(SUM(lv.LvAmount/8.0),0) as lvAmount from #tblLvHistory lv inner join tblLeaveType lt on lv.LeaveCode = lt.LeaveCode and lt.PaidRate > 0 and lv.LeaveDate between @FromDate and @ToDate group by lv.EmployeeID) lv
on a.EmployeeID = lv.EmployeeID
and AllowanceRuleID in (19)
--ngay cong di lam thuc te + ngay chu nhat
UPDATE #tblAllowance SET TotalPaidDays = ISNULL(a.TotalPaidDays,0) + ISNULL(att.TotalSunDay,0)
from #tblAllowance a
inner join (select EmployeeID,SUM(CASE WHEN WorkingTime > Std_Hour_PerDays THEN 1 ELSE WorkingTime/Std_Hour_PerDays END) as TotalSunDay from #Tadata where HolidayStatus > 0 group by EmployeeID) att on a.EmployeeID = att.EmployeeID
and AllowanceRuleID in (10)
-- rule 20 nhan voi so ngay Chu nhan di lam du cong

UPDATE #tblAllowance SET TotalPaidDays = isnull(FullWorkingTimeSunDays,0)
from #tblAllowance a
inner join (select EmployeeID,COUNT(1) as FullWorkingTimeSunDays from #Tadata where WorkingTime >= Std_Hour_PerDays AND HolidayStatus = 1 group by EmployeeID) att on a.EmployeeID = att.EmployeeID
and AllowanceRuleID in (20)

--So ngay di lam tron cong (lam du 8h cong 1 ngay moi dc tinh 1 lan tra)
UPDATE #tblAllowance SET TotalPaidDays = isnull(FullWorkingTimeDays,0)
from #tblAllowance a
inner join (select EmployeeID,COUNT(1) as FullWorkingTimeDays from #Tadata where WorkingTime >= Std_Hour_PerDays group by EmployeeID) att on a.EmployeeID = att.EmployeeID
and AllowanceRuleID in (18)
...
--hưởng trọn gói
UPDATE #tblAllowance SET
ReceiveAmount = CASE WHEN ISNULL(TotalPaidDays,0) = 0 THEN 0 ELSE ISNULL(DefaultAmount,0) END
...
where AllowanceRuleID IN (3,4,5,7,8)


UPDATE al SET
	ReceiveAmount = DefaultAmount*TotalPaidDays
 from #tblAllowance al
inner join tblSalaryCalculationRule sc
on al.SalCalRuleID = sc.SalCalRuleID --and isnull(sc.IsDaily,0)=0
...
select s.Code,s.Qty,s.DeductPercentage,cast (0 as float) as ActualQty, cast(0 as float(53)) DeductAmount,a.* into #tblDiligentAll from #tblAllowance a, tblDiligentAllowanceRule s
where a.AllowanceRuleID = 9
order by a.EmployeeID,s.Code,s.PaidPercentage desc

--ngay cong di lam thuc te
UPDATE #tblDiligentAll SET TotalPaidDays = ISNULL(att.AttDays,0) + ISNULL(att.ProbationHours,0)/8.0
from #tblDiligentAll a
	inner join #tblSal_AttendanceData att on a.EmployeeID = att.EmployeeID
	where a.Code = 'PDAWD'

-- Đi trễ về - sớm bao nhiêu phút
SELECT EmployeeID,
 IODate,IOKind,IOMinutes,IOMinutesDeduct,ApprovedDeduct
	 INTO #tblIOData
FROM tblInLateOutEarly
WHERE ApprovedDeduct = 1 and ApprovedDeduct > 0
 AND IODate BETWEEN @FromDate AND @ToDate
 AND EmployeeID IN (SELECT EmployeeID FROM #tblEmployeeIDList)
...
INNER JOIN
 (SELECT EmployeeID, SUM(IOMinutesDeduct) AS IOMinute FROM #tblIOData
 GROUP BY EmployeeID) tmp ON d.EmployeeID = tmp.EmployeeID
...
INNER JOIN
 ...
 Count(1) NotOTCount
 FROM tblNotOtList
 WHERE OTDate BETWEEN @FromDate AND @ToDate
 AND Approved = 1
 AND EmployeeID IN
 (SELECT EmployeeID
...
INNER JOIN
 ...
 lv.LeaveCode,
 SUM(lv.LvAmount)/8.0 AS lvCount
 FROM #tblDiligentAll d
 INNER JOIN #tblLvHistory lv ON d.EmployeeID = lv.EmployeeID
 AND d.Code = lv.LeaveCode
 WHERE Code NOT IN ('PD','PDAWD',
 'IOMinute',
 'IOCount',
 'NotOT',
 'FWCCount',
...
WHERE isnull(NotAllowDiligence,0) = 1)
 GROUP BY lv.EmployeeID,
 lv.LeaveCode) TMP ON d.EmployeeID = tmp.EmployeeID
AND d.Code = tmp.LeaveCode



UPDATE #tblDiligentAll SET ActualQty = tmp.lvCount
FROM #tblDiligentAll D


INNER JOIN (
	SELECT lv.EmployeeID,lv.lvCount FROM #tblDiligentAll d
	INNER JOIN (select SUM(lv.LvAmount)/8.0 AS lvCount,EmployeeID
		from #tblLvHistory lv where isnull(lv.NotAllowDiligence ,0)= 1
		GROUP BY lv.EmployeeID
	) lv ON d.EmployeeID = lv.EmployeeID) TMP ON d.EmployeeID = tmp.EmployeeID
	WHERE Code IN ('ALL_leave')



DELETE #tblDiligentAll WHERE Qty > ActualQty

DELETE d FROM #tblDiligentAll d
WHERE NOT EXISTS
 ...
 (SELECT 1 FROM
		...
		FROM #tblDiligentAll WHERE Qty <= ActualQty GROUP BY Code, EmployeeID)tmp
 WHERE d.EmployeeID = tmp.EmployeeID AND d.Code = tmp.Code AND d.Qty = tmp.Qty

)

UPDATE #tblDiligentAll SET DeductAmount = DefaultAmount*DeductPercentage/100

-- Get the remain of diligent allowance
-- Negative set to zero
UPDATE d SET ReceiveAmount = CASE
 WHEN tmp.DefaultAmount - tmp.DeductAmount > 0 THEN tmp.DefaultAmount - tmp.DeductAmount ELSE 0
		END
FROM #tblDiligentAll d
INNER JOIN
 (SELECT d.EmployeeID,
 Max(d.DefaultAmount) DefaultAmount,
 SUM(d.DeductAmount) DeductAmount
 FROM #tblDiligentAll d
 GROUP BY d.EmployeeID) tmp ON d.EmployeeID = tmp.EmployeeID
...
FROM #tblAllowance a
left JOIN #tblDiligentAll d ON a.EmployeeID = d.EmployeeID
AND a.AllowanceID = d.AllowanceID
where a.AllowanceRuleID = 9
...
group by a.EmployeeID) adj on sal.EmployeeID = adj.EmployeeID
 ...
 UPDATE #tblSalDetail_ForTax
 set TotalIncome = round(isnull(ActualMonthlyBasic,0) + isnull(TotalOTAmount,0)+ isnull(TotalNSAmt,0) + ISNULL(TotalAllowanceForSalary,0) + isnull(TotalAdjustmentForSalary,0) + isnull(OtherDeductionAfterPIT,0)
...
begin
	...
	FROM #TableVarTax txv, (SELECT * FROM tblTax tt WHERE tt.EffectDate = (SELECT MAX(EffectDate) FROM tblTax WHERE datediff(day,EffectDate, @FromDate )>=0)) tx
...
END
INSERT INTO tblSC_EZLog(LoginID, LogTime, EmployeeID, FunctionClassName, FunctionName, KindOfData, DateOfData, OldData, NewData)
select @LoginID,getdate(),'ALL', 'Process salary', 'SALCAL_MAIN', 'OK',CAST(@year AS VARCHAR) + '/' + CAST(@Month AS VARCHAR) + '/1', '','Process salary'
print 'eof'
